...
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'oportunidades_contas' AND schema_id = SCHEMA_ID('teste'))
BEGIN
    CREATE TABLE teste..oportunidades_contas (
...
-- Inserir dados de exemplo para abertura-conta
-- Verificar se já existem dados para não duplicar
IF NOT EXISTS (SELECT TOP 1 * FROM teste..oportunidades_contas WHERE TIPO_ESTRATEGIA = 'abertura-conta')
BEGIN
    ...
    VALUES (
        ...
        @AdminUserId, 'Carlos Oliveira', 'Centro SP', 'abertura-conta'
    );
    ...
    VALUES (
        ...
        @AdminUserId, 'Ana Pereira', 'Vila Olímpia', 'abertura-conta'
    );
    ...
    VALUES (
        ...
        @AdminUserId, 'Roberto Costa', 'Campinas Shop', 'abertura-conta'
    );
    ...
    VALUES (
        ...
        @AdminUserId, 'Paulo Mendes', 'Rio Branco', 'abertura-conta'
    );
    ...
    VALUES (
        ...
        @AdminUserId, 'Marcos Vieira', 'Salvador Shop', 'abertura-conta'
    );
    ...
    VALUES (
        ...
        @AdminUserId, 'Camila Rocha', 'BH Centro', 'abertura-conta'
    );
    
    PRINT 'Dados de exemplo inseridos para abertura-conta.';
END
ELSE
BEGIN
    PRINT 'Já existem dados para abertura-conta. Nenhum dado inserido.';
END

-- Criar índices para melhorar performance
IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'IX_oportunidades_contas_tipo_estrategia' AND object_id = OBJECT_ID('teste..oportunidades_contas'))
BEGIN
    CREATE INDEX IX_oportunidades_contas_tipo_estrategia ON teste..oportunidades_contas (TIPO_ESTRATEGIA);
    PRINT 'Índice para TIPO_ESTRATEGIA criado.';
END

IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'IX_oportunidades_contas_user_id' AND object_id = OBJECT_ID('teste..oportunidades_contas'))
BEGIN
    CREATE INDEX IX_oportunidades_contas_user_id ON teste..oportunidades_contas (USER_ID);
    PRINT 'Índice para USER_ID criado.';
END

IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'IX_oportunidades_contas_chave_loja' AND object_id = OBJECT_ID('teste..oportunidades_contas'))
BEGIN
    CREATE INDEX IX_oportunidades_contas_chave_loja ON teste..oportunidades_contas (CHAVE_LOJA);
